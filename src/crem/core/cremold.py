...
from crem.utils.mol_context import combine_core_env_to_rxn_smarts, get_canon_context_core, patt_remove_map
...
def __extend_output_by_equivalent_atoms(mol, output):
    ...
    extended_output = []
    for item in output:
        if all(i in atom_eq for i in item[2]):  # if all atoms of a fragment have equivalent atoms
...
def __fragment_mol_link(
    mol1, mol2, radius=3, keep_stereo=False, protected_ids_1=None, protected_ids_2=None, return_ids=True
):
    ...
    for r in reactants:
        ...
        for y in ps:
            for p in y:
                ...
                if e:
                    sys.stderr.write(
                        "Molecule %s caused sanitization error %i" % (Chem.MolToSmiles(p, isomericSmiles=True), e)
                    )
                    sys.stderr.flush()
                else:
    ...
    **kwargs,
    ...
    if link:
        f = __fragment_mol_link(
            mol1=mol1, mol2=mol2, radius=radius, protected_ids_1=protected_ids_1, protected_ids_2=protected_ids_2
        )
    ...
    if f:
        ...
        for env, core, *ids in f:  # if link = True ids is two tuples, if link = False ids is a single tuple
            ...
            if (min_size <= num_heavy_atoms <= max_size and min_rel_size <= hac_ratio <= max_rel_size) or (
                replace_cycles and cycle_pattern.search(core)
            ):
            ...
            for row_id, core_smi, core_sma, freq in res:
                if core_smi != replacements[row_id][1]:
                    if return_frag_smi_only:
                        ...
                        yield (
                            replacements[row_id][0],
                            core_sma,
                            freq,
                            replacements[row_id][2][0],
                            replacements[row_id][2][1],
                        )
                    else:
...
def get_replacements(
    ...
    **kwargs,
    ...
    for frag_smi in __gen_replacements(
        mol1=mol1,
        mol2=mol2,
        db_name=db_name,
        radius=radius,
